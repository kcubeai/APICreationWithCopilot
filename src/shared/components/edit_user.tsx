...
import { Button, Checkbox, Form, Input, Layout,Radio, notification} from "antd";
...
import { useRouter } from "next/router";
    ...
    export default function editUser({data, onClose}:any){
        ...
        // const [userisadmin, setUserisadmin] = useState<boolean>(data.userDetail.userList[0].isadmin);
            ...
            <Content style={{ padding: "50px", display: 'flex', justifyContent: 'center', alignContent: 'center' }}>
                ...
                <div style={{ width: "50%" }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '16px' }} className="form-control">
                            ...
                            <Input defaultValue={getValues("name")}  {...register("name", { required: true })} onChange={(newValue) => setValue("name", newValue.target.value)} />
                        </div>
...
<Radio.Group
      ...
      >
        ...
        <Radio value='userisuser'   {...register('userisuser')} 
                 onChange={handleRadioChange}   >User is User</Radio>
      </Radio.Group>
...
{!getValues("userisadmin") && (
 ...
 <Button htmlType="submit" disabled={!watch("projectList") || !(watch("ec2Instances")?.length || watch("rdsIdentifiers")?.length || watch("vmInstances")?.length)}>
                            Edit User
                        </Button>    
