...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (authorization) {
        if (authorization !== process.env.APP_KEY) {
            ...
            //         const aws_ec2_init_update = `update ec2_instances, project_id=null where project_id=${id}`;
            ...
            else if (action =="edit")
            {
                ...
                try{
                    ...
                    const updateQuery = `update projects set project_name='${name}' where id=${userID}`

                    const result1 = DBCONNECT(updateQuery);
                    // console.log("insert query executed", result);
                    // const project_name = await DBCONNECT(`select project_name from projects where id=${id}`);
                    // const delete_project = await DBCONNECT(`delete from service_assigned_with_projects where project_id=${id}`);
                    // const aws_ec2_init_update = `update ec2_instances, project_id=null where project_id=${id}`;
                    // const aws_rds_init_update = `update rds_identifiers , project_id=null where project_id=${id}`;

                    // if (ec2Instances && ec2Instances.length > 0) {
                    //     const aws_ec2_string = ec2Instances.map((value: any) => `'${value}'`).join(', ');
                    //     console.log(ec2Instances)
                       
                    //     const service_assigned_query = await DBCONNECT(`insert into service_assigned_with_projects(service_id,service_type,project_id) SELECT unnest(ARRAY[${aws_ec2_string}]), 'ec2',${id};`)
                    //     console.log(service_assigned_query.rows);
                    // }
                    // if (rdsIdentifiers && rdsIdentifiers.length > 0) {
                    //     const aws_rds_string = rdsIdentifiers.map((value: any) => `'${value}'`).join(', ');
                    ...
                    //     const service_assigned_query = await DBCONNECT(`insert into service_assigned_with_projects(service_id,service_type,project_id) SELECT unnest(ARRAY[${aws_rds_string}]), 'rds',${id};`)
                    //     console.log(service_assigned_query.rows);
                    // }
                    // if (vmInstances && vmInstances.length > 0) {
                    //     const gcp_vm_string = vmInstances.map((value: any) => `'${value}'`).join(', ');

                    //     const service_assigned_query = await DBCONNECT(`insert into service_assigned_with_projects(service_id,service_type,project_id) SELECT unnest(ARRAY[${gcp_vm_string}]), 'vm',${id};`)
                    //     console.log(service_assigned_query);
                    // }
                    res.status(200).json({ message: 'Project Updated Successfully' })
                } catch (error) {
